...
from dc_iteration.data import write_directory, write_jsonl, load_solutions
from dc_iteration.syncheck import syntax_check
...
def get_deps(nodes: List[Tuple[str, Node]]) -> dict:
...
def get_function_dependency(entrypoint: str, call_graph: dict) -> Set[str]:
    queue = [entrypoint]
    visited = {entrypoint}
    while queue:
        current = queue.pop(0)
...
def extract_target_code_or_empty(code: str, entrypoint: Optional[str] = None) -> str:
    ...
    for child in root_node.children:
        if child.type in IMPORT_TYPE:
            ...
            name = get_definition_name(child)
            if name not in function_names and name not in variable_names and name not in class_names and has_return_statement(child):
                definition_nodes.append((name, child))
                function_names.add(get_definition_name(child))
        elif child.type == EXPRESSION_TYPE and child.children[0].type == ASSIGNMENT_TYPE:
            subchild = child.children[0]
            name = get_definition_name(subchild)
            if name not in variable_names and name not in function_names and name not in class_names:
